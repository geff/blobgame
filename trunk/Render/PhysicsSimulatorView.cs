    ...
    public class PhysicsSimulatorView
    {
        ...
        private void DrawContacts(SpriteBatch spriteBatch)
        {
            ...
            for (int i = 0; i < _physicsSimulator.ArbiterList.Count; i++)
            {
                ...
                {
                    _contactCircleBrush.Draw(spriteBatch, _physicsSimulator.ArbiterList[i].ContactList[j].Position * factor + centerScreen);
                }
            }
        }

        public Vector2 centerScreen = new Vector2(400, 300);
            ...
            for (int i = 0; i < _physicsSimulator.GeomList.Count; i++)
            {
                ...
                for (int j = 0; j < verticeCount; j++)
                {
                    if (_enableEdgeView)
                    {
                        if (j < verticeCount - 1)
                        {
                            _edgeLineBrush.Draw(spriteBatch, _physicsSimulator.GeomList[i].WorldVertices[j]*factor+centerScreen,
                                                _physicsSimulator.GeomList[i].WorldVertices[j + 1]*factor+centerScreen);
                        }
                        else
                        {
                            _edgeLineBrush.Draw(spriteBatch, _physicsSimulator.GeomList[i].WorldVertices[j]*factor+centerScreen,
                                                _physicsSimulator.GeomList[i].WorldVertices[0]*factor+centerScreen);
                        }
                    }
                    if (_enableVerticeView)
                    {
                        _verticeCircleBrush.Draw(spriteBatch, _physicsSimulator.GeomList[i].WorldVertices[j] * factor + centerScreen);
                    }
                }
            }
        }
        ...
        private void DrawSprings(SpriteBatch spriteBatch)
        {
            for (int i = 0; i < _physicsSimulator.SpringList.Count; i++)
            {
                ...
                _springLineBrush.Draw(spriteBatch, _body1AttachPointInWorldCoordinates * factor + centerScreen,
                                      fixedLinearSpring.WorldAttachPoint * factor + centerScreen);
            }
            ...
            for (int i = 0; i < _physicsSimulator.SpringList.Count; i++)
            {
                ...
                _springCircleBrush.Draw(spriteBatch, _body1AttachPointInWorldCoordinates * factor + centerScreen);
                _springCircleBrush.Draw(spriteBatch, _body2AttachPointInWorldCoordinates * factor + centerScreen);

                Vector2.Lerp(ref _body1AttachPointInWorldCoordinates, ref _body2AttachPointInWorldCoordinates, .25f,
                             out _vectorTemp1);
                _springCircleBrush.Draw(spriteBatch, _vectorTemp1 * factor + centerScreen);
                ...
                _springLineBrush.Draw(spriteBatch, _body1AttachPointInWorldCoordinates * factor + centerScreen,
                                      _body2AttachPointInWorldCoordinates * factor + centerScreen);
            }
        }
        ...
        private void DrawRevoluteJoints(SpriteBatch spriteBatch)
        {
            for (int i = 0; i < _physicsSimulator.JointList.Count; i++)
            {
                if (_physicsSimulator.JointList[i] is FixedRevoluteJoint)
                {
                    ...
                    _revoluteJointRectangleBrush.Draw(spriteBatch, fixedRevoluteJoint.Anchor * factor + centerScreen);
                }
                ...
                _revoluteJointRectangleBrush.Draw(spriteBatch, revoluteJoint.CurrentAnchor * factor + centerScreen);
                _revoluteJointLineBrush.Draw(spriteBatch, revoluteJoint.CurrentAnchor * factor + centerScreen, revoluteJoint.Body1.Position * factor + centerScreen);
                _revoluteJointLineBrush.Draw(spriteBatch, revoluteJoint.CurrentAnchor * factor + centerScreen, revoluteJoint.Body2.Position * factor + centerScreen);
            }
        }
        ...
        private void DrawPinJoints(SpriteBatch spriteBatch)
        {
            for (int i = 0; i < _physicsSimulator.JointList.Count; i++)
            {
                ...
                _pinJointRectangleBrush.Draw(spriteBatch, pinJoint.WorldAnchor1 * factor + centerScreen);
                _pinJointRectangleBrush.Draw(spriteBatch, pinJoint.WorldAnchor2 * factor + centerScreen);
                _pinJointLineBrush.Draw(spriteBatch, pinJoint.WorldAnchor1 * factor + centerScreen, pinJoint.WorldAnchor2 * factor + centerScreen);
            }
        }
        ...
        private void DrawSliderJoints(SpriteBatch spriteBatch)
        {
            for (int i = 0; i < _physicsSimulator.JointList.Count; i++)
            {
                ...
                _sliderJointRectangleBrush.Draw(spriteBatch, sliderJoint.WorldAnchor1 * factor + centerScreen);
                _sliderJointRectangleBrush.Draw(spriteBatch, sliderJoint.WorldAnchor2 * factor + centerScreen);
                _sliderJointLineBrush.Draw(spriteBatch, sliderJoint.WorldAnchor1 * factor + centerScreen, sliderJoint.WorldAnchor2 * factor + centerScreen);
            }
        }
    }
}